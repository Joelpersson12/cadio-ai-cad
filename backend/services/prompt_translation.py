...
_PHRASE_TRANSLATIONS: tuple[tuple[str, str], ...] = (
    ...
    # English synonyms / common alternate phrasings
    (r"\b(?:power\s*wash(?:er)?|jet\s*wash(?:er)?|pressure\s*clean(?:er)?|karcher)\b", "pressure washer"),
    (r"\b(?:hose\s*(?:clip|clamp|strap|tie|bracket))\b", "hose holder"),
    (r"\b(?:wire\s*(?:clip|tie|management|tidy)|cable\s*(?:tidy|management|raceway))\b", "cable organizer"),
    (r"\b(?:earphone|earbud|airpod)s?\s*(?:case|holder|stand|box)\b", "earphone case"),
    (r"\b(?:sd\s*card|micro\s*sd|memory\s*card)\s*(?:holder|case|organiz(?:er)?|storage)\b", "sd card holder"),
    (r"\b(?:usb\s*hub|hub)\s*(?:holder|stand|mount)\b", "usb hub mount"),
    (r"\b(?:spool\s*holder|filament\s*spool|filament\s*holder|reel\s*holder)\b", "filament spool holder"),
    (r"\b(?:nozzle|jet)\s*(?:holder|mount|bracket)\b", "nozzle holder"),
    (r"\b(?:spray\s*(?:bottle|can)|aerosol)\s*(?:holder|rack|mount)\b", "spray bottle holder"),
    (r"\b(?:garden\s*hose|water\s*hose|pressure\s*hose)\b", "hose"),
    (r"\b(?:desk\s*organiz(?:er|ation)|office\s*organiz(?:er|ation)|desktop\s*organiz(?:er|ation))\b", "desk organizer"),
    (r"\b(?:pill|tablet|medication)\s*(?:organiz(?:er|er)|dispenser|case|box)\b", "pill organizer"),
    (r"\b(?:magnetic|magnet)\s*(?:mount|holder|dock|base)\b", "magnetic mount"),
    (r"\b(?:pipe\s*(?:clip|clamp|holder|bracket)|conduit\s*(?:clip|holder))\b", "pipe clamp"),
    (r"\b(?:vent|air\s*vent|ac\s*vent|hvac)\s*(?:clip|mount|holder|bracket)\b", "vent mount"),
    (r"\b(?:raspberry\s*pi|rpi)\s*(?:case|enclosure|mount|holder)\b", "raspberry pi case"),
    (r"\b(?:gopro|action\s*cam(?:era)?)\s*(?:mount|holder|stand|bracket)\b", "gopro mount"),
    (r"\b(?:ring\s*light|led\s*ring)\s*(?:stand|mount|holder)\b", "ring light stand"),
