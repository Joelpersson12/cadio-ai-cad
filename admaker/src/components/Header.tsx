    ...
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4"
      ...
      >
        ...
        <span className="font-bold text-base tracking-tight text-white group-hover:gradient-text transition-all">
          AdForge<span className="text-brand-400"> AI</span>
        </span>
      </button>
      ...
      {!minimal && (
        <nav className="hidden md:flex items-center gap-8 text-sm text-white/60">
          ...
          <a href="https://cadio.app" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">Cadio AI</a>
        </nav>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={onStart} className="btn-primary text-xs px-4 py-2">
          {minimal ? '← Back to Home' : 'Create Free Ad'}
        </button>
      </div>
    </header>
  )
