...
export default function Header({ onStart, minimal, onBack, user, onSignIn, onSignOut }: HeaderProps) {
  return (
        ...
        {user ? (
          <>
            <div className="hidden sm:flex items-center gap-2">
              ...
              <span className="text-white/70 text-sm">{user.name.split(' ')[0]}</span>
            </div>
            <button onClick={onSignOut} className="btn-ghost text-xs px-4 py-2">Sign out</button>
          </>
        ) : (
          !minimal && <button onClick={onSignIn} className="btn-ghost text-xs px-4 py-2">Sign in</button>
        )}
        <button onClick={minimal ? (onBack ?? onStart) : onStart} className="btn-primary text-xs px-4 py-2">
